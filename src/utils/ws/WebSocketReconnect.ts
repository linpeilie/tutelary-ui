...
export default class WebSocketReconnect implements IWebSocketReconnect {
  ...
  constructor(websocket: IWebSocketBean) {
    ...
    this.reconnectGapTime = websocket.param.reconnectGapTime ?? 30000;
  }
  ...
  start = () => {
    ...
    this.num = 0;
    if (this.websocket.param.onReconnect) this.websocket.param.onReconnect();
    this.timer = setInterval(() => {
      if (this.reconnectMaxNum && this.num >= this.reconnectMaxNum) {
        if (this.websocket.param.onFailReconnect) this.websocket.param.onFailReconnect();
        this.stop();
        return;
      }
