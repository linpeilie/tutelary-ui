...
export const LoggerAppender = {
  encode(message: LoggerAppender, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.file !== '') writer.uint32(18).string(message.file);

    if (message.blocking !== '') writer.uint32(26).string(message.blocking);

    for (const v of message.appenderRef) writer.uint32(34).string(v!);

    if (message.target !== '') writer.uint32(42).string(message.target);
