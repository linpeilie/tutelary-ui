...
export interface ProfilerStartResponse {
  sessionId: string;
  status: string;
  jobId: number;
  state: number;
...
function createBaseProfilerStartResponse(): ProfilerStartResponse {
  return { sessionId: "", status: "", jobId: 0, state: 0, message: "" };
}

export const ProfilerStartResponse = {
  decode(input: _m0.Reader | Uint8Array, length?: number): ProfilerStartResponse {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseProfilerStartResponse();
