...
export const ThreadDetail = {
  encode(message: ThreadDetail, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.threadState !== '') writer.uint32(26).string(message.threadState);

    if (message.lock !== undefined) LockInfo.encode(message.lock, writer.uint32(34).fork()).ldelim();

    if (message.lockName !== '') writer.uint32(42).string(message.lockName);

    if (message.lockOwnerId !== 0) writer.uint32(48).int64(message.lockOwnerId);

    if (message.lockOwnerName !== '') writer.uint32(58).string(message.lockOwnerName);

    for (const v of message.stackTrace) StackTraceNode.encode(v!, writer.uint32(66).fork()).ldelim();

    if (message.jobId !== 0) writer.uint32(72).int32(message.jobId);

    if (message.state !== 0) writer.uint32(80).int32(message.state);

    if (message.message !== '') writer.uint32(90).string(message.message);
