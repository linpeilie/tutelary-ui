...
export const FileListResponse = {
  encode(message: FileListResponse, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.total !== 0) writer.uint32(8).int32(message.total);

    for (const v of message.fileList) FileInfo.encode(v!, writer.uint32(18).fork()).ldelim();

    if (message.jobId !== 0) writer.uint32(24).int32(message.jobId);

    if (message.state !== 0) writer.uint32(32).int32(message.state);

    if (message.message !== '') writer.uint32(42).string(message.message);
