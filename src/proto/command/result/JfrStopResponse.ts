...
export interface JfrStopResponse {
  recordingId: string;
  status: string;
  jobId: number;
  state: number;
...
function createBaseJfrStopResponse(): JfrStopResponse {
  return { recordingId: "", status: "", jobId: 0, state: 0, message: "" };
}

export const JfrStopResponse = {
  decode(input: _m0.Reader | Uint8Array, length?: number): JfrStopResponse {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseJfrStopResponse();
